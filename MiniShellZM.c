   ...
   for (cmd = cmds; cmd != NULL; cmd = cmd->next) {
      ...
      if ((childPID = fork()) < 0)
         ...
         if (outFD < 0 && cmd->outFile[0]) {  /* If no out-pipe, but a redirect */
            if (cmd->flags == OVER_WRITE) {
               outFD = open(cmd->outFile, O_WRONLY|O_CREAT|O_TRUNC, 0644);
            }
            else if (cmd->flags == RIGHT_CHEVRON) {
               outFD = open(cmd->outFile, O_WRONLY|O_CREAT|O_EXCL, 0644);
               if (outFD < 0 && errno == EEXIST) {
                 fprintf(stderr, "%s", strerror(EEXIST));
               }
            }
